 ...
 * Implements audit log (logging) data access operations
 * 
 * The data access layer that stores, queries, summarizes, and manages history
 */
...
export class AuditLogRepository implements Repository<AuditLog, any, any, any> {
  ...
  async findMany(filters?: AuditLogFiltersDTO): Promise<AuditLog[]> {
    try {
      ...
      return result.map((auditLog: any) => this.transformAuditLog(auditLog));
    } catch (error) {
  ...
  async findManyPaginated(
    ...
    try {
      ...
      return {
        data: data.map((auditLog: any) => this.transformAuditLog(auditLog)),
        pagination: {
          page,
          limit,
          total,
    ...
    try {
      ...
      const [auditLog] = await this.getDb()
        ...
        .values({
          ...
          userId: data.userId,
          action: data.action,
          // details is the details of the audit log like what was done, 
  ...
  async getAuditStats(): Promise<{
    ...
    try {
      ...
      const logsByAction = actionStats.reduce((acc: any, stat: any) => {
  ...
  async getDocumentAuditSummary(
    ...
    try {
      /*
      filters by documentId, and optionally by data range if: dateFrom, dateTo
      are provided. 
      */
      ...
      /*
      queries the database via findmany to get all audit log entries that match
      those filters.
  ...
  details: {
    message: 'User viewed document',
    user: 'user-123',
    document: 'doc-1',
    timestamp: '2025-10-07T19:22:00Z'
  }
  }
      ...
      */
      ...
      /*
      acc: is an accumulator, it is an object that we use to store the 
      events by action

      It groups and counts logs by their action type.
      Basically, it says “how many times each type of action occurred.”
