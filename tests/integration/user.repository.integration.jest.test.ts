...
import { users } from '../../src/app/infrastructure/database/models/schema';
...
describe('User Repository Integration Tests', () => {
  ...
  describe('Basic CRUD Operations', () => {
    it('should create and find user by ID', async () => {
      ...
      await dbConfig.db.insert(users).values({
        ...serializedUser,
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle unique email constraint', async () => {
      ...
      const serializedUser1 = userRepository.serializeToDatabase(user1);
      await dbConfig.db.insert(users).values({
        ...serializedUser1,
        password: 'hashed-password',
      });
      ...
      const serializedUser2 = userRepository.serializeToDatabase(user2);
      await expect(
        dbConfig.db.insert(users).values({
          ...serializedUser2,
          password: 'hashed-password',
        })
      ).rejects.toThrow();
