...
describe("DocumentVersionEntity", () => {
  ...
  it("should validate version number", async () => {
    ...
    await expect(runEffect(DocumentVersionEntity.create(badInput))).rejects.toThrow(DocumentVersionValidationError)
  })

  it("should detect first version", async () => {
    const v1 = await runEffect(DocumentVersionEntity.create(createFirstVersion("doc-id" as any)))
  ...
  it("should handle creator info properly", async () => {
    const withCreator = await runEffect(DocumentVersionEntity.create(createVersionWithCreator("user-1" as any)))
  ...
  it("should handle property-based valid data", async () => {
    await fc.assert(
